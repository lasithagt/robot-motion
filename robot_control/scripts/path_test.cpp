...
void read_data(int &n_data_q, std::unique_ptr<Eigen::MatrixXf> &data_q) {
    ...
    n_data_q = read_txt_files::readFile_nlines(getenv("HOME") + std::string("/Documents/ROS_ws/src/kuka_motion/robot_control/data/desired_position_FM_dynamic_circle.txt"));
    ...
    int ret = read_txt_files::readFile(*(data_q.get()), getenv("HOME") + std::string("/Documents/ROS_ws/src/kuka_motion/robot_control/data/desired_position_FM_dynamic_circle.txt"));
...
int main (int argc, char** argv) {
    ...
    robot_motion_primitives::robot_motion robot_position = robot_motion_primitives::robot_motion(*robot_, kuka_robot, 7, 600, vel_prof_name);
    ...
    while (ros::ok() && !ros::isShuttingDown() && (new_time < duration)) {
