...
void read_data(int &n_data_q, std::unique_ptr<Eigen::MatrixXf> &data_q) {
    ...
    n_data_q = read_txt_files::readFile_nlines(getenv("HOME") + std::string("/Documents/ROS_ws/src/kuka-motion/data/desired_position_FM.txt"));
    ...
    int ret = read_txt_files::readFile(*(data_q.get()), getenv("HOME") + std::string("/Documents/ROS_ws/src/kuka-motion/data/desired_position_FM.txt"));
...
int main (int argc, char** argv) {
    ...
    while (ros::ok() || !ros::isShuttingDown()) {
		...
		if (robot_.getRobotIsConnected()) {
    		...
    		if (!static_pos) {
				Eigen::MatrixXf data = (*data_).block(0,0,7,n_data_q); 
	    		// robot_position.robot_motion::move_spline_to(data, position, duration);
	    		ROS_INFO_STREAM("Starting the trajectory...");
	    		robot_position.robot_motion::spline_pp(data, position, duration);
	    	}
