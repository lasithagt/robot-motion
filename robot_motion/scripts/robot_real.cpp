...
void read_data(int &n_data_q, std::unique_ptr<Eigen::MatrixXf> &data_q) {
    ...
    n_data_q = read_txt_files::readFile_nlines(getenv("HOME") + std::string("/Documents/ROS_ws/src/kuka-motion/data/identification_friction_straight.txt"));
    ...
    int ret = read_txt_files::readFile(*(data_q.get()), getenv("HOME") + std::string("/Documents/ROS_ws/src/kuka-motion/data/identification_friction_straight.txt"));
...
int main (int argc, char** argv) {
	...
	const double rate_interp = 300.0;
    ...
    while (ros::ok() && !ros::isShuttingDown() && (new_time < splines_[0].domain().back()) && (new_time < duration)) {
		...
		wrench_ref.wrench.quantity[1] = -4;
